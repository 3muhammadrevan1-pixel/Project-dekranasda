        ...
        @foreach ($organisasi as $s)
            <div class="col-12 col-sm-6 col-md-4 d-flex justify-content-center">
                <div class="card h-100 text-center p-4" style="max-width:320px;">
                    {{-- Menggunakan 'img' dari TbMenuData --}}
                    ...
                    {{-- PERBAIKAN: Hapus [0]. Akses 'jabatan' langsung dari organisasi_data --}}
                    <p class="text-muted mb-2">
                        {{ $s->organisasi_data['jabatan'] ?? 'Jabatan tidak tersedia' }}
                    </p>

                    {{-- PERBAIKAN: Hapus [0]. Akses 'deskripsi' langsung dari organisasi_data --}}
                    <p class="struktur-desc">
                        {{ $s->organisasi_data['deskripsi'] ?? 'Deskripsi tidak tersedia' }}
                    </p>
                </div>
            </div>
        @endforeach
