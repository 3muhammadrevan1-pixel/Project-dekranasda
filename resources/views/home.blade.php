            ...
            <div class="modal-content p-0 border-0 rounded-3 shadow-lg overflow-hidden position-relative">
              <div class="row g-0">
                ...
                <div class="col-md-6 p-4 d-flex flex-column">
                  ...
                  <h5 class="text-danger fw-bold mb-3" id="price{{ $pr->id }}">{{ number_format($productPrice,0,',','.') }}</h5>
                    ...
                    <div class="mb-3">
                      ...
                      <div class="d-flex flex-wrap gap-2">
                        @foreach($pr->variants as $variant)
                          <button type="button"
                                  class="btn btn-outline-dark btn-sm color-btn"
                                  data-product="{{ $pr->id }}"
...
function selectSize(productId, size){
  ...
  const btn = Array.from(sizesContainer.querySelectorAll('.size-option')).find(b=>b.innerText==size);
...
document.addEventListener('click', function(e){
 if(e.target.classList.contains('color-btn')){
   ...
   // Update gambar utama
    ...
    // Dapatkan sumber gambar langsung dari atribut data tombol
    const newImgSrc = e.target.dataset.img; 
    
     if(mainImg && newImgSrc) {
        mainImg.src = newImgSrc; // <-- Ubah ke ini
    }

    // Update harga
    const priceEl = document.getElementById('price'+productId);
    // Pastikan formatRupiah dipanggil dan harga adalah angka, bukan string
